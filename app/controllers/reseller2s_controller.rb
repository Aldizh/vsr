...
class Reseller2sController < ApplicationController  
  ...
  def addNewTariffSubmit
    ...
    begin
      ...
      redirect_to "/reseller2s/addNewTariff"
    end
  end
  ...
  def createTariffSubmit
    ...
    identifier = DB[:resellers2].where(:id => session[:current_reseller2_id]).first[:identifier]
    ...
    begin
      ...
      DB[:tariffreseller].insert(:id_tariff => @id_tariff, :id_reseller => session[:current_reseller2_id], :resellerlevel => 2)

      flash[:notice_added] = "Tariff successfully created!"
      redirect_to "/reseller2s/agentsTariffs"
    rescue
      flash[:error_adding] = "Tariff could not created! Try again! or contact the administrator."
      redirect_to "/reseller2s/createTariff"
    end    
  end
  ...
  def getAgentTariffs
    my_agents_tariffs = []
    reseller_tariffs = DB[:tariffreseller].where(:id_reseller => session[:current_reseller2_id])
    id_tariff_array = []
